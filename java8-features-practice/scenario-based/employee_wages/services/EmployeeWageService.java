...
import employee_wages.repository.CompanyRepository;

public class EmployeeWageService implements IEmployeeWageService {
    ...
    public void addCompany(String name, int wagePerHour,int maxDays, int maxHours) 
    {
    	...
    	repository.addCompany(new CompanyEmpWage(name, wagePerHour, maxDays, maxHours));
    }
	...
	@Override
	public void computeWage(CompanyEmpWage company) {
	 ...
	 public int getTotalWageByCompany(String name) {
	        CompanyEmpWage company = repository.getCompanyByName(name);
